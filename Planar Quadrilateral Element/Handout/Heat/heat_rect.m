function[fT]=heat_rect(h,E,v,a,T,mm,MP,i)
   syms s t

            m1=mm(1,1);  m2=mm(1,2); m3=mm(1,3); m4=mm(1,4);
            x1=MP(m1,1);y1=MP(m1,2);
            x2=MP(m2,1);y2=MP(m2,2);  %节点坐标
            x3=MP(m3,1);y3=MP(m3,2);
            x4=MP(m4,1);y4=MP(m4,2);
          X=[x1;x2;x3;x4]; Y=[y1;y2;y3;y4];
          st=[-0.5773 -0.5773;-0.5773 0.5773;0.5773 -0.5773;0.5773 0.5773];
          % 高斯积分的四个高斯点坐标
          D=E/(1-v^2)*[1 v 0;v 1 0;0 0 (1-v)/2]; % D平面应力
          yb=[a*T;a*T;0];  %热应变
          fT=zeros(8,1);   %单元刚度阵初始阵
   ...
   FT=h*J*(B')*D*yb; %第n个高斯点下计算得到的热应力
   fT=fT+FT;  %把单个高斯点算出的热应力加和，得到总热应力
  end
